...
(async () => {
  ...
  await fs.writeFile("epilepsy-clinicals-potential-cases.csv", "patient-id,dob,codes,last-encounter\n");
  for(let patient in patients) {
    try {
      ...
      await fs.appendFile("epilepsy-clinicals-potential-cases.csv", row);
    } catch(error) {
      console.log(error);
    }
  }
